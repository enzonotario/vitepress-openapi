...
describe('useTheme', () => {
  ...
  it('handles Ref values in requestBodyDefaultView without double-wrapping', () => {
    const viewRef = ref('schema')
    themeConfig.setRequestBodyDefaultView(viewRef)

    const result = themeConfig.getRequestBodyDefaultView()
    expect(result).toBe('schema')

    // If we update the ref, it shouldn't affect the value since it's been unwrapped
    viewRef.value = 'contentType'
    expect(themeConfig.getRequestBodyDefaultView()).toBe('schema')
  })
  ...
  it('handles Ref values in jsonViewerDeep without double-wrapping', () => {
    const deepRef = ref(10)
    themeConfig.setJsonViewerDeep(deepRef)
    ...
    // If we update the ref, it shouldn't affect the value since it's been unwrapped
    deepRef.value = 20
    expect(themeConfig.getJsonViewerDeep()).toBe(10)
  })
  ...
  it('handles Ref values in i18n config without double-wrapping', () => {
    const localeRef = ref('ja')

    themeConfig.setI18nConfig({ locale: localeRef })

    const result = themeConfig.getI18nConfig()

    expect(result.locale.value).toBe('ja')

    // Verify the Ref is not double-wrapped.
    // If it's double-wrapped, result.locale.value would be a Ref itself.
    expect(isRef(result.locale.value)).toBe(false)
  })
  ...
  it('handles Ref values in spec config without double-wrapping', () => {
    const groupByTagsRef = ref(false)
    const collapsePathsRef = ref(true)
    const showPathsSummaryRef = ref(false)
    const avoidCircularsRef = ref(true)
    const lazyRenderingRef = ref(true)

    themeConfig.setSpecConfig({
      groupByTags: groupByTagsRef,
      collapsePaths: collapsePathsRef,
      showPathsSummary: showPathsSummaryRef,
      avoidCirculars: avoidCircularsRef,
      lazyRendering: lazyRenderingRef,
    })

    const result = themeConfig.getSpecConfig()

    expect(result.groupByTags.value).toBe(false)
    expect(result.collapsePaths.value).toBe(true)
    expect(result.showPathsSummary.value).toBe(false)
    expect(result.avoidCirculars.value).toBe(true)
    expect(result.lazyRendering.value).toBe(true)

    // Verify the Refs are not double-wrapped
    // If they're double-wrapped, result.*.value would be a Ref itself
    expect(isRef(result.groupByTags.value)).toBe(false)
    expect(isRef(result.collapsePaths.value)).toBe(false)
    expect(isRef(result.showPathsSummary.value)).toBe(false)
    expect(isRef(result.avoidCirculars.value)).toBe(false)
    expect(isRef(result.lazyRendering.value)).toBe(false)
  })
...
describe('config updates do not duplicate refs', () => {
  const themeConfig = useTheme()
  ...
  it('updates existing refs when setting the same property multiple times', () => {
    themeConfig.setRequestBodyDefaultView('schema')
    themeConfig.setRequestBodyDefaultView('contentType')
    themeConfig.setRequestBodyDefaultView('autogenerated')

    const updatedState = themeConfig.getState()

    expect(updatedState.requestBody.defaultView).toBe('autogenerated')
  })

  it('updates nested refs correctly', () => {
    themeConfig.setPlaygroundJsonEditorMode('text')
    themeConfig.setPlaygroundJsonEditorMainMenuBar(true)
    themeConfig.setPlaygroundJsonEditorNavigationBar(true)
    ...
    expect(updatedState.playground.jsonEditor.mode).toBe('text')
    expect(updatedState.playground.jsonEditor.mainMenuBar).toBe(true)
    expect(updatedState.playground.jsonEditor.navigationBar).toBe(true)
    expect(updatedState.playground.jsonEditor.statusBar).toBe(true)
  })

  it('updates i18n config correctly', () => {
    themeConfig.setI18nConfig({ locale: 'es' })
    themeConfig.setI18nConfig({ locale: 'ja' })
    themeConfig.setI18nConfig({ locale: 'pt-BR' })

    const updatedState = themeConfig.getState()

    expect(updatedState.i18n.locale).toBe('pt-BR')
  })

  it('updates spec config correctly', () => {
    themeConfig.setSpecConfig({ groupByTags: false })
    themeConfig.setSpecConfig({ collapsePaths: true })
    themeConfig.setSpecConfig({ showPathsSummary: false })

    const updatedState = themeConfig.getState()

    expect(updatedState.spec.groupByTags).toBe(false)
    expect(updatedState.spec.collapsePaths).toBe(true)
    expect(updatedState.spec.showPathsSummary).toBe(false)
  })

  it('updates multiple config sections correctly', () => {
    themeConfig.setRequestBodyDefaultView('schema')
    themeConfig.setJsonViewerDeep(5)
    themeConfig.setSchemaViewerDeep(3)
    themeConfig.setI18nConfig({ locale: 'es' })
    ...
    themeConfig.setRequestBodyDefaultView('contentType')
    themeConfig.setJsonViewerDeep(10)
    themeConfig.setSchemaViewerDeep(6)
    themeConfig.setI18nConfig({ locale: 'ja' })
    themeConfig.setSpecConfig({ groupByTags: true })

    const updatedState = themeConfig.getState()

    expect(updatedState.requestBody.defaultView).toBe('contentType')
    expect(updatedState.jsonViewer.deep).toBe(10)
    expect(updatedState.schemaViewer.deep).toBe(6)
    expect(updatedState.i18n.locale).toBe('ja')
    expect(updatedState.spec.groupByTags).toBe(true)
  })

  it('preserves other properties when updating specific ones', () => {
    themeConfig.setRequestBodyDefaultView('schema')
    themeConfig.setJsonViewerDeep(5)
    themeConfig.setSchemaViewerDeep(3)

    const initialState = themeConfig.getState()
    ...
    expect(updatedState.requestBody.defaultView).toBe('contentType')
    expect(updatedState.jsonViewer.deep).toBe(initialState.jsonViewer.deep)
    expect(updatedState.schemaViewer.deep).toBe(initialState.schemaViewer.deep)
  })
})
