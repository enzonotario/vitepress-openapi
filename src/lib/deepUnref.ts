 ...
 */
export const deepUnref = <T>(val: T): DeepUnref<T> => {
 ...
 */
const smartUnref = <T>(val: T): DeepUnref<T> => {
 ...
 */
const unrefArray = <T>(arr: T[]): DeepUnref<T>[] =>
  arr.map(item => smartUnref(item))
 ...
 */
const unrefObject = <T extends Record<string, unknown>>(obj: T): DeepUnrefObject<T> => {
