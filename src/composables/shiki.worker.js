    ...
    try {
        switch (type) {
            ...
            case 'load-lang':
                ...
                self.postMessage({ type: 'load-lang-success', id, lang: payload.lang })
                break

            case 'loaded-lang':
                if (!shiki) throw new Error('Shiki not initialized')
                self.postMessage({ type: 'loaded-lang-success', id, lang: payload.lang, loaded: shiki.getLoadedLanguages().includes(payload.lang) })
                break

            case 'render':
                if (!shiki) throw new Error('Shiki not initialized')
                const html = shiki.codeToHtml(payload.content, {
                    lang: payload.lang,
                    theme: payload.theme,
                })
                self.postMessage({ type: 'render-success', id, html })
                break
            
            case 'reset':
                shiki = null
                languageLoadPromises.clear()
                self.postMessage({type: 'reset-success', id })
                break
