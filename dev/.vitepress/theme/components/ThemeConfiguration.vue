...
const availableSlots = DEFAULT_OPERATION_SLOTS

const slots = ref([])

watch(slots, (newSlots) => {
  themeConfig.setOperationHiddenSlots(newSlots)
}, { deep: true })
</script>
...
<template>
  <div
    :class="{
      'sticky top-[64px] max-h-[250px] z-50 overflow-y-auto': sticky,
    }"
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
          <label v-for="slot in availableSlots" :key="slot" class="flex items-center gap-2">
            <input
              type="checkbox"
              :checked="slots.includes(slot)"
              @change="slots.includes(slot) ? slots.splice(slots.indexOf(slot), 1) : slots.push(slot)"
            >
            {{ slot }}
          </label>
        </div>
